...
import PostReactionModel from '../models/postReaction';
  ...
  async (req, res) => {
    try {
      ...
      const response = await Promise.all(
        files.map((file) =>
          uploadBytes(storageRef(v4()), file.buffer, {
      ...
      const createPost = await PostModel.findOneAndUpdate(
...
router.post('/:postId/reaction', verifyToken, async (req, res) => {
  try {
    ...
    const updatePostReact = await PostModel.findOneAndUpdate(
      { _id: postId },
      {
        $inc: inc,
      },
...
router.get('/:postId/reaction', verifyToken, async (req, res) => {
  try {
    ...
    const comments = await CommentModel.find({ post: postId }).count();
